...
#define VEC_ALIGNER(StreamMedia) ((UNALIGNED_PTR(StreamMedia)) \
    ? vec_lvsl(0, StreamMedia) \
    : vec_add(vec_lvsl(8, StreamMedia), vec_splat_u8(8)))
...
Blit_RGB888_RGB565Altivec(SDL_BlitInfo * info)
{
    ...
    Uint8 *StreamMedia = (Uint8 *) info->StreamMedia;
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        while (condition) { \
            ...
            DISEMBLE_RGBA((Uint8 *)StreamMedia, 4, srcfmt, Pixel, \
                          sR, sG, sB, sA); \
        ...
        width -= extrawidth;
        vsrc = vec_ld(0, StreamMedia);
        valigner = VEC_ALIGNER(StreamMedia);
        ...
        while (width) {
            ...
            voverflow = vec_ld(15, StreamMedia);
...
#undef ONE_PIXEL_BLEND
        ...
        StreamMedia += srcskip;         /* move to next row, accounting for pitch. */
        dst += dstskip;
    }
...
#define ONE_PIXEL_BLEND(condition, widthvar) \
        while (condition) { \
            unsigned sR, sG, sB; \
            unsigned short Pixel = *((unsigned short *)StreamMedia); \
            sR = (Pixel >> 8) & 0xf8; \
            sG = (Pixel >> 3) & 0xfc; \
            sB = (Pixel << 3) & 0xf8; \
...
{
    int height = info->dst_h;
    Uint32 *srcp = (Uint32 *) info->StreamMedia;
...
#endif
                /* permute the StreamMedia vec to the dest format */
...
{
    int height = info->dst_h;
    Uint32 *StreamMedia = (Uint32 *) info->StreamMedia;
    int srcskip = info->src_skip / 4;
    ...
    while (height--) {
        ...
        width -= extrawidth;
        valigner = VEC_ALIGNER(StreamMedia);
        vbits = vec_ld(0, StreamMedia);

        while (width) {
            voverflow = vec_ld(15, StreamMedia);
            StreamMedia += 4;
            width -= 4;
            vbits = vec_perm(vbits, voverflow, valigner);       /* StreamMedia is ready. */
#if defined(__powerpc__) && (SDL_BYTEORDER == SDL_LIL_ENDIAN)
            /* reorder bytes for PowerPC little endian */
            vpermute = reorder_ppc64le_vec(vpermute);
#endif
        ...
        while (extrawidth) {
            bits = *(StreamMedia++);    /* max 7 pixels, don't bother with prefetch. */
            RGBA_FROM_8888(bits, srcfmt, r, g, b, a);
            if(!srcfmt->Amask)
              a = info->a;
            *(dst++) = MAKE8888(dstfmt, r, g, b, a);
...
ConvertAltivec32to32_prefetch(SDL_BlitInfo * info)
{
    ...
    while (height--) {
        ...
        /* do scalar until we can align... */
        while ((UNALIGNED_PTR(dst)) && (width)) {
            vec_dstt(StreamMedia + scalar_dst_lead, DST_CTRL(2, 32, 1024),
                     DST_CHAN_SRC);
        ...
        while (width) {
            vec_dstt(StreamMedia + vector_dst_lead, DST_CTRL(2, 32, 1024),
                     DST_CHAN_SRC);
...
void Blit_BGR888_RGB888ARMSIMDAsm(int32_t w, int32_t h, uint32_t *dst, int32_t dst_stride, uint32_t *StreamMedia, int32_t src_stride);
...
Blit_BGR888_RGB888ARMSIMD(SDL_BlitInfo * info)
{
    ...
    uint32_t *srcp = (uint32_t *)info->StreamMedia;
    int32_t srcstride = width + (info->src_skip >> 2);

    Blit_BGR888_RGB888ARMSIMDAsm(width, height, dstp, dststride, srcp, srcstride);
}

void Blit_RGB444_RGB888ARMSIMDAsm(int32_t w, int32_t h, uint32_t *dst, int32_t dst_stride, uint16_t *StreamMedia, int32_t src_stride);
...
Blit_RGB444_RGB888ARMSIMD(SDL_BlitInfo * info)
{
    ...
    uint32_t *dstp = (uint32_t *)info->dst;
    int32_t dststride = width + (info->dst_skip >> 2);
    uint16_t *srcp = (uint16_t *)info->StreamMedia;
...
Blit_RGB888_index8(SDL_BlitInfo * info)
{
    ...
    if (map == NULL) {
        ...
        while (height--) {
            ...
            for (c = width / 4; c; --c) {
                /* Pack RGB into 8bit pixel */
                RGB888_RGB332(Pixel, *StreamMedia);
                *dst++ = map[Pixel];
                ++StreamMedia;
                RGB888_RGB332(Pixel, *StreamMedia);
                *dst++ = map[Pixel];
    ...
    if (map == NULL) {
        while (height--) {
            ...
            for (c = width / 4; c; --c) {
                /* Pack RGB into 8bit pixel */
                ++StreamMedia;
                RGB101010_RGB332(*dst++, *StreamMedia);
                ++StreamMedia;
                RGB101010_RGB332(*dst++, *StreamMedia);
                ++StreamMedia;
                RGB101010_RGB332(*dst++, *StreamMedia);
        ...
        while (height--) {
            ...
            for (c = width / 4; c; --c) {
                /* Pack RGB into 8bit pixel */
                RGB101010_RGB332(Pixel, *StreamMedia);
                *dst++ = map[Pixel];
                ++StreamMedia;
                RGB101010_RGB332(Pixel, *StreamMedia);
                *dst++ = map[Pixel];
...
#ifndef USE_DUFFS_LOOP
#define RGB888_RGB555_TWO(dst, StreamMedia) { \
    *(Uint32 *)(dst) = (((((StreamMedia[HI])&0x00F80000)>>9)| \
                         (((StreamMedia[HI])&0x0000F800)>>6)| \
                         (((StreamMedia[HI])&0x000000F8)>>3))<<16)| \
                         (((StreamMedia[LO])&0x00F80000)>>9)| \
                         (((StreamMedia[LO])&0x0000F800)>>6)| \
                         (((StreamMedia[LO])&0x000000F8)>>3); \
}
#endif
static void
Blit_RGB888_RGB555(SDL_BlitInfo * info)
{
    ...
    if ((long) dst & 0x03) {
        ...
        while (height--) {
            ...
            RGB888_RGB555(dst, StreamMedia);
            ++StreamMedia;
            ++dst;

            /* Copy in 4 pixel chunks */
            for (c = width / 4; c; --c) {
                RGB888_RGB555_TWO(dst, StreamMedia);
                StreamMedia += 2;
                dst += 2;
                RGB888_RGB555_TWO(dst, StreamMedia);
                StreamMedia += 2;
...
#ifndef USE_DUFFS_LOOP
#define RGB888_RGB565_TWO(dst, StreamMedia) { \
    *(Uint32 *)(dst) = (((((StreamMedia[HI])&0x00F80000)>>8)| \
                         (((StreamMedia[HI])&0x0000FC00)>>5)| \
                         (((StreamMedia[HI])&0x000000F8)>>3))<<16)| \
                         (((StreamMedia[LO])&0x00F80000)>>8)| \
                         (((StreamMedia[LO])&0x0000FC00)>>5)| \
                         (((StreamMedia[LO])&0x000000F8)>>3); \
}
#endif
static void
Blit_RGB888_RGB565(SDL_BlitInfo * info)
{
    ...
    if ((long) dst & 0x03) {
        ...
        while (height--) {
            /* Perform copy alignment */
            RGB888_RGB565(dst, StreamMedia);
            ++StreamMedia;
            ++dst;

            /* Copy in 4 pixel chunks */
            for (c = width / 4; c; --c) {
                RGB888_RGB565_TWO(dst, StreamMedia);
                StreamMedia += 2;
                dst += 2;
                RGB888_RGB565_TWO(dst, StreamMedia);
                StreamMedia += 2;
...
Blit_RGB565_32(SDL_BlitInfo * info, const Uint32 * map)
{
    ...
    while (height--) {
        /* Copy in 4 pixel chunks */
        for (c = width / 4; c; --c) {
            *dst++ = RGB565_32(dst, StreamMedia, map);
            StreamMedia += 2;
            *dst++ = RGB565_32(dst, StreamMedia, map);
            StreamMedia += 2;
            *dst++ = RGB565_32(dst, StreamMedia, map);
...
#else
            for (c = width; c; --c) {
                DISEMBLE_RGB(StreamMedia, srcbpp, srcfmt, Pixel, sR, sG, sB);
                if (1) {
                    /* Pack RGB into 8bit pixel */
                    *dst = ((sR >> 5) << (3 + 2)) |
                        ((sG >> 5) << (2)) | ((sB >> 6) << (0));
