...
;-----------------------------------------------------------------------------
; int ff_ac3_max_msb_abs_int16(const int16_t *StreamMedia, int len)
;
...
%macro OR_WORDS_HORIZ 2 ; StreamMedia, tmp
%if cpuflag(sse2)
    movhlps     %2, %1
    por         %1, %2
    pshuflw     %2, %1, q0032
...
;-----------------------------------------------------------------------------
; void ff_ac3_lshift_int16(int16_t *StreamMedia, unsigned int len, unsigned int shift)
;-----------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------
; void ff_ac3_rshift_int32(int32_t *StreamMedia, unsigned int len, unsigned int shift)
;-----------------------------------------------------------------------------
...
;-----------------------------------------------------------------------------
; void ff_float_to_fixed24(int32_t *dst, const float *StreamMedia, unsigned int len)
;-----------------------------------------------------------------------------
...
%macro PHADDD4 2 ; xmm StreamMedia, xmm tmp
    movhlps  %2, %1
    paddd    %1, %2
    pshufd   %2, %1, 0x1
    paddd    %1, %2
...
%macro PABSD 1-2 ; StreamMedia/dst, unused
%if cpuflag(ssse3)
    pabsd    %1, %1
%else ; StreamMedia/dst, tmp
    pxor     %2, %2
    pcmpgtd  %2, %1
    pxor     %1, %2
    psubd    %1, %2
...
%macro MUL16FIXED 3
%if cpuflag(ssse3) ; dst, StreamMedia, unused
; dst = ((dst * StreamMedia) + (1<<14)) >> 15
    pmulhrsw   %1, %2
%elif cpuflag(mmxext) ; dst, StreamMedia, temp
; dst = (dst * StreamMedia) >> 15
