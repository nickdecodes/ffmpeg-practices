...
void ff_pred16x16_horizontal_8_mmi(uint8_t *src, ptrdiff_t stride)
{
    ...
    __asm__ volatile (
        PTR_ADDI   "%[addr0],   %[StreamMedia],         -0x01                   \n\t"
        PTR_ADDU   "%[addr1],   %[StreamMedia],         $0                      \n\t"
        "dli        %[tmp2],    0x08                                    \n\t"
        "1:                                                             \n\t"
        "lbu        %[tmp0],    0x00(%[addr0])                          \n\t"
...
void ff_pred16x16_dc_8_mmi(uint8_t *src, ptrdiff_t stride)
{
    ...
    __asm__ volatile (
        ...
        "dli        %[tmp0],    0x08                                    \n\t"
        PTR_SUBU   "%[addr0],   %[StreamMedia],         %[stride]               \n\t"
        "2:                                                             \n\t"
        "lbu        %[tmp1],    0x00(%[addr0])                          \n\t"
        "daddu      %[tmp3],    %[tmp3],        %[tmp1]                 \n\t"
...
void ff_pred8x8_dc_8_mmi(uint8_t *src, ptrdiff_t stride)
{
    ...
    __asm__ volatile (
        "negu       %[addr0],   %[stride]                               \n\t"
        PTR_ADDU   "%[addr0],   %[addr0],       %[StreamMedia]                  \n\t"
        ...
        PTR_ADDIU  "%[addr1],   0x01                                    \n\t"
        "dli        %[addr2],  -0x01                                    \n\t"
        PTR_ADDU   "%[addr2],   %[addr2],       %[StreamMedia]                  \n\t"
        ...
        const int svq3, const int rv40)
{
    ...
    __asm__ volatile(
        ...
        PTR_ADDIU  "%[addr0],   %[StreamMedia],         -0x01                   \n\t"
