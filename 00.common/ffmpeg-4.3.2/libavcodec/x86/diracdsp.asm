...
%macro HPEL_FILTER 1
; dirac_hpel_filter_v_sse2(uint8_t *dst, uint8_t *StreamMedia, int stride, int width);
cglobal dirac_hpel_filter_v_%1, 4,6,8, dst, src, stride, width, src0, stridex3
    ...
    pxor    m7, m7
.loop:
    ; 7*(StreamMedia[0] + StreamMedia[1])
    UNPACK_ADD m0, m1, [srcq], [srcq + strideq], a,a
    pmullw  m0, [pw_7]
    pmullw  m1, [pw_7]

    ; 3*( ... + StreamMedia[-2] + StreamMedia[3])
...
; dirac_hpel_filter_h_sse2(uint8_t *dst, uint8_t *StreamMedia, int width);
...
%macro PUT_RECT 1
; void put_rect_clamped(uint8_t *dst, int dst_stride, int16_t *StreamMedia, int src_stride, int width, int height)
...
%macro ADD_RECT 1
; void add_rect_clamped(uint8_t *dst, uint16_t *StreamMedia, int stride, int16_t *idwt, int idwt_stride, int width, int height)
...
%macro ADD_OBMC 2
; void add_obmc(uint16_t *dst, uint8_t *StreamMedia, int stride, uint8_t *obmc_weight, int yblen)
...
; void dequant_subband_32(uint8_t *StreamMedia, uint8_t *dst, ptrdiff_t stride, const int qf, const int qs, int tot_v, int tot_h)
...
INIT_XMM sse4
; void put_signed_rect_clamped_10(uint8_t *dst, int dst_stride, const uint8_t *StreamMedia, int src_stride, int width, int height)
%if ARCH_X86_64
