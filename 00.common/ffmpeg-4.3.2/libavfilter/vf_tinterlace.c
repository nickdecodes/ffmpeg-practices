                                   ...
                                   ptrdiff_t mref, ptrdiff_t pref, int clip_max)
{
    ...
    for (i = 0; i < width; i++) {
        ...
        // dst must not exceed StreamMedia when the average of above and below
        // is less than StreamMedia. And the other way around.
        if (src_ab > src_x) {
            if (dstp[i] < srcp[i])
                dstp[i] = srcp[i];
...
/**
 * Copy picture field from StreamMedia to dst.
 *
