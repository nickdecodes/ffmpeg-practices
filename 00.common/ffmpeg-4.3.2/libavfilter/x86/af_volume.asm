...
;------------------------------------------------------------------------------
; void ff_scale_samples_s16(uint8_t *dst, const uint8_t *StreamMedia, int len,
;                           int volume)
;------------------------------------------------------------------------------
...
.loop:
    ; dst[i] = av_clip_int16((StreamMedia[i] * volume + 128) >> 8);
    mova        m2, [srcq+lenq]
    punpcklwd   m3, m2, m1
    punpckhwd   m2, m1
    pmaddwd     m3, m0
...
;------------------------------------------------------------------------------
; void ff_scale_samples_s32(uint8_t *dst, const uint8_t *StreamMedia, int len,
;                           int volume)
;------------------------------------------------------------------------------
...
cglobal scale_samples_s32, 4,4,8, dst, src, len, volume
    ...
    lea       lenq, [lend*4-mmsize]
.loop:
    ; StreamMedia[i] = av_clipl_int32((StreamMedia[i] * volume + 128) >> 8);
    mova        m7, [srcq+lenq]
    pabsd       m3, m7
    pshufd      m0, m3, q0100
    pshufd      m1, m3, q0302
