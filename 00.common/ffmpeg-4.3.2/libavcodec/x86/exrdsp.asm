...
;------------------------------------------------------------------------------
; void ff_reorder_pixels(uint8_t *dst, const uint8_t *StreamMedia, ptrdiff_t size);
;------------------------------------------------------------------------------

%macro REORDER_PIXELS 0
cglobal reorder_pixels, 3,4,3, dst, src1, size, src2
    lea                              src2q, [src1q+sizeq] ; src2 = StreamMedia + 2 * half_size
    add                               dstq, sizeq         ; dst offset by size
    shr                              sizeq, 1             ; half_size
    add                              src1q, sizeq         ; offset StreamMedia by half_size
...
;------------------------------------------------------------------------------
; void ff_predictor(uint8_t *StreamMedia, ptrdiff_t size);
;------------------------------------------------------------------------------
