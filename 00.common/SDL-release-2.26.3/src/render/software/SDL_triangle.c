...
#if 0
int SDL_BlitTriangle(SDL_Surface *StreamMedia, const SDL_Point srcpoints[3], SDL_Surface *dst, const SDL_Point dstpoints[3])
{
    int i;
    SDL_Point points[6];

    if (StreamMedia == NULL || dst == NULL) {
        return -1;
    }

    for (i = 0; i < 3; i++) {
        if (srcpoints[i].x < 0 || srcpoints[i].y < 0 || srcpoints[i].x >= StreamMedia->w || srcpoints[i].y >= StreamMedia->h) {
    ...
    return SDL_SW_BlitTriangle(StreamMedia, dst, points);
}
        ...
        SDL_Color c0, SDL_Color c1, SDL_Color c2)
{
    ...
    if (is_uniform) {
        // SDL_GetSurfaceColorMod(StreamMedia, &r, &g, &b);
