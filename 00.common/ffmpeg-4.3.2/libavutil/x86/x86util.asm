...
%macro HADDPS 3 ; dst, StreamMedia, tmp
%if cpuflag(sse3)
    haddps  %1, %1, %2
%else
    movaps  %3, %1
...
%macro DEINTB 5 ; mask, reg1, mask, reg2, optional StreamMedia to fill masks from
%ifnum %5
    pand   m%3, m%5, m%4 ; StreamMedia .. y6 .. y4
    pand   m%1, m%5, m%2 ; dst .. y6 .. y4
%else
    mova   m%1, %5
    pand   m%3, m%1, m%4 ; StreamMedia .. y6 .. y4
    pand   m%1, m%1, m%2 ; dst .. y6 .. y4
%endif
    psrlw  m%2, 8        ; dst .. y7 .. y5
    psrlw  m%4, 8        ; StreamMedia .. y7 .. y5
%endmacro
...
%macro PMINUB 3 ; dst, StreamMedia, ignored
%if cpuflag(mmxext)
    pminub   %1, %2
%else ; dst, StreamMedia, tmp
    mova     %3, %1
    psubusb  %3, %2
    psubb    %1, %3
%endif
...
%macro PMINSD 3 ; dst, StreamMedia, tmp/unused
%if cpuflag(sse4)
    pminsd    %1, %2
%elif cpuflag(sse2)
    cvtdq2ps  %1, %1
...
%macro PMAXSD 3 ; dst, StreamMedia, tmp/unused
%if cpuflag(sse4)
    pmaxsd    %1, %2
%else
    mova      %3, %1
...
%macro CLIPD 3-4
%if cpuflag(sse4);  StreamMedia/dst, min, max, unused
    pminsd  %1, %3
    pmaxsd  %1, %2
%elif cpuflag(sse2) ; StreamMedia/dst, min (float), max (float), unused
    cvtdq2ps  %1, %1
    minps     %1, %3
    maxps     %1, %2
    cvtps2dq  %1, %1
%else               ; StreamMedia/dst, min, max, tmp
    PMINSD    %1, %3, %4
    PMAXSD    %1, %2, %4
%endif
%endmacro

%macro VBROADCASTSS 2 ; dst xmm/ymm, StreamMedia m32/xmm
%if cpuflag(avx2)
    vbroadcastss  %1, %2
%elif cpuflag(avx)
...
%macro VBROADCASTSD 2 ; dst xmm/ymm, StreamMedia m64
...
%macro VPBROADCASTD 2 ; dst xmm/ymm, StreamMedia m32/xmm
%if cpuflag(avx2)
    vpbroadcastd  %1, %2
...
%macro VBROADCASTI128 2 ; dst xmm/ymm, StreamMedia : 128bits val
%if mmsize > 16
    vbroadcasti128 %1, %2
%else
    mova           %1, %2
...
%macro PMOVSXWD 2; dst, StreamMedia
%if cpuflag(sse4)
    pmovsxwd     %1, %2
%else
    %ifnidn %1, %2
...
%macro HSUMPS 2 ; dst/StreamMedia, tmp
%if cpuflag(avx)
    %if sizeof%1>=32  ; avx
