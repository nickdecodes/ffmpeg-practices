...
typedef struct SwsContext {
    ...
    /**
     * Note that StreamMedia, dst, srcStride, dstStride will be copied in the
     ...
     *         dst[i] += StreamMedia[ filterPos[i] + j ] * filter[ filterSize * i + j ];
...
// warp input lines in the form (StreamMedia + width*i + j) to slice format (line[i][j])
// relative=true means first line StreamMedia[x][0] otherwise first line is StreamMedia[x][lum/crh Y]
