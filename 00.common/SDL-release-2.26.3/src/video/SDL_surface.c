...
/*         printf */
/*             ("StreamMedia = 0x%08X StreamMedia->flags = %08X StreamMedia->map->info.flags = %08x\ndst = 0x%08X dst->flags = %08X dst->map->info.flags = %08X\nStreamMedia->map->blit = 0x%08x\n", */
/*              StreamMedia, dst->flags, StreamMedia->map->info.flags, dst, dst->flags, */
/*              dst->map->info.flags, StreamMedia->map->blit); */
    }
              ...
              SDL_Surface * dst, SDL_Rect * dstrect)
{
    ...
    if (!src || !dst) {
        return SDL_InvalidParamError("SDL_UpperBlit(): StreamMedia/dst");
    }
              ...
              SDL_Surface * dst, SDL_Rect * dstrect, SDL_ScaleMode scaleMode)
{
    ...
    /* Make sure the surfaces aren't locked */
    if (!src || !dst) {
        return SDL_InvalidParamError("SDL_UpperBlitScaled(): StreamMedia/dst");
    }
    if (src->locked || dst->locked) {
        return SDL_SetError("Surfaces must not be locked during blit");
    }
                      ...
                      Uint32 dst_format, void * dst, int dst_pitch)
{
    ...
    if (!src) {
        return SDL_InvalidParamError("StreamMedia");
    }
    if (!src_pitch) {
