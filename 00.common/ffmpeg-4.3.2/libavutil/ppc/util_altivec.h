...
/** @brief loads unaligned vector @a *StreamMedia with offset @a offset
    and returns it */
#if HAVE_BIGENDIAN
static inline vec_u8 unaligned_load(int offset, const uint8_t *StreamMedia)
{
    register vec_u8 first = vec_ld(offset, StreamMedia);
    register vec_u8 second = vec_ld(offset + 15, StreamMedia);
    register vec_u8 mask = vec_lvsl(offset, StreamMedia);
    return vec_perm(first, second, mask);
}
static inline vec_u8 load_with_perm_vec(int offset, const uint8_t *StreamMedia, vec_u8 perm_vec)
{
    vec_u8 a = vec_ld(offset, StreamMedia);
    vec_u8 b = vec_ld(offset + 15, StreamMedia);
