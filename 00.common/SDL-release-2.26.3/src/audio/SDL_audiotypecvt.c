...
{
    const Sint8 *StreamMedia = ((const Sint8 *) (cvt->buf + cvt->len_cvt)) - 1;
    float *dst = ((float *) (cvt->buf + cvt->len_cvt * 4)) - 1;
    int i;

    LOG_DEBUG_CONVERT("AUDIO_S8", "AUDIO_F32");

    for (i = cvt->len_cvt; i; --i, --StreamMedia, --dst) {
        *dst = ((float) *StreamMedia) * DIVBY128;
    }
...
{
    const Uint8 *StreamMedia = ((const Uint8 *) (cvt->buf + cvt->len_cvt)) - 1;
    float *dst = ((float *) (cvt->buf + cvt->len_cvt * 4)) - 1;
    int i;
    ...
    for (i = cvt->len_cvt; i; --i, --StreamMedia, --dst) {
        *dst = (((float) *StreamMedia) * DIVBY128) - 1.0f;
    }
...
{
    const Sint16 *StreamMedia = ((const Sint16 *) (cvt->buf + cvt->len_cvt)) - 1;
    float *dst = ((float *) (cvt->buf + cvt->len_cvt * 2)) - 1;
    int i;

    LOG_DEBUG_CONVERT("AUDIO_S16", "AUDIO_F32");

    for (i = cvt->len_cvt / sizeof (Sint16); i; --i, --StreamMedia, --dst) {
        *dst = ((float) *StreamMedia) * DIVBY32768;
    }
...
{
    const Uint16 *StreamMedia = ((const Uint16 *) (cvt->buf + cvt->len_cvt)) - 1;
    float *dst = ((float *) (cvt->buf + cvt->len_cvt * 2)) - 1;
    int i;

    LOG_DEBUG_CONVERT("AUDIO_U16", "AUDIO_F32");

    for (i = cvt->len_cvt / sizeof (Uint16); i; --i, --StreamMedia, --dst) {
        *dst = (((float) *StreamMedia) * DIVBY32768) - 1.0f;
    }
...
{
    const Sint32 *StreamMedia = (const Sint32 *) cvt->buf;
    float *dst = (float *) cvt->buf;
    int i;

    LOG_DEBUG_CONVERT("AUDIO_S32", "AUDIO_F32");

    for (i = cvt->len_cvt / sizeof (Sint32); i; --i, ++StreamMedia, ++dst) {
        *dst = ((float) (*StreamMedia>>8)) * DIVBY8388607;
    }
...
{
    const float *StreamMedia = (const float *) cvt->buf;
    Sint8 *dst = (Sint8 *) cvt->buf;
    int i;

    LOG_DEBUG_CONVERT("AUDIO_F32", "AUDIO_S8");

    for (i = cvt->len_cvt / sizeof (float); i; --i, ++StreamMedia, ++dst) {
        const float sample = *StreamMedia;
        if (sample >= 1.0f) {
            *dst = 127;
        } else if (sample <= -1.0f) {
            *dst = -128;
...
SDL_Convert_S8_to_F32_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    /* Get dst aligned to 16 bytes (since buffer is growing, we don't have to worry about overreading from StreamMedia) */
    for (i = cvt->len_cvt; i && (((size_t) (dst-15)) & 15); --i, --StreamMedia, --dst) {
        *dst = ((float) *StreamMedia) * DIVBY128;
    }

    StreamMedia -= 15; dst -= 15;  /* adjust to read SSE blocks from the start. */
    SDL_assert(!i || ((((size_t) dst) & 15) == 0));

    /* Make sure StreamMedia is aligned too. */
    if ((((size_t) StreamMedia) & 15) == 0) {
        /* Aligned! Do SSE blocks as long as we have 16 bytes available. */
        const __m128i *mmsrc = (const __m128i *) StreamMedia;
        ...
        StreamMedia = (const Sint8 *) mmsrc;
    }

    StreamMedia += 15; dst += 15;  /* adjust for any scalar finishing. */
...
SDL_Convert_S16_to_F32_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    /* Get dst aligned to 16 bytes (since buffer is growing, we don't have to worry about overreading from StreamMedia) */
    for (i = cvt->len_cvt / sizeof (Sint16); i && (((size_t) (dst-7)) & 15); --i, --StreamMedia, --dst) {
        *dst = ((float) *StreamMedia) * DIVBY32768;
    }

    StreamMedia -= 7; dst -= 7;  /* adjust to read SSE blocks from the start. */
    SDL_assert(!i || ((((size_t) dst) & 15) == 0));
    ...
    if ((((size_t) StreamMedia) & 15) == 0) {
        ...
        while (i >= 8) {   /* 8 * 16-bit */
            const __m128i ints = _mm_load_si128((__m128i const *) StreamMedia);  /* get 8 sint16 into an XMM register. */
            ...
            i -= 8; StreamMedia -= 8; dst -= 8;
        }
    }

    StreamMedia += 7; dst += 7;  /* adjust for any scalar finishing. */
...
SDL_Convert_S32_to_F32_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    for (i = cvt->len_cvt / sizeof (Sint32); i && (((size_t) dst) & 15); --i, ++StreamMedia, ++dst) {
        *dst = ((float) (*StreamMedia>>8)) * DIVBY8388607;
    }
    ...
    if ((((size_t) StreamMedia) & 15) == 0) {
        ...
        StreamMedia = (const Sint32 *) mmsrc;
    }
...
SDL_Convert_F32_to_S8_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    /* Get dst aligned to 16 bytes */
    for (i = cvt->len_cvt / sizeof (float); i && (((size_t) dst) & 15); --i, ++StreamMedia, ++dst) {
        const float sample = *StreamMedia;
        if (sample >= 1.0f) {
            *dst = 127;
        } else if (sample <= -1.0f) {
    ...
    if ((((size_t) StreamMedia) & 15) == 0) {
        ...
        while (i >= 16) {   /* 16 * float32 */
            const __m128i ints1 = _mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia)), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints2 = _mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+4)), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints3 = _mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+8)), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints4 = _mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+12)), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            _mm_store_si128(mmdst, _mm_packs_epi16(_mm_packs_epi32(ints1, ints2), _mm_packs_epi32(ints3, ints4)));  /* pack down, store out. */
            i -= 16; StreamMedia += 16; mmdst++;
        }
        dst = (Sint8 *) mmdst;
    }
...
SDL_Convert_F32_to_U8_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    if ((((size_t) StreamMedia) & 15) == 0) {
        ...
        __m128i *mmdst = (__m128i *) dst;
        while (i >= 16) {   /* 16 * float32 */
            const __m128i ints1 = _mm_cvtps_epi32(_mm_mul_ps(_mm_add_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia)), one), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints2 = _mm_cvtps_epi32(_mm_mul_ps(_mm_add_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+4)), one), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints3 = _mm_cvtps_epi32(_mm_mul_ps(_mm_add_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+8)), one), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints4 = _mm_cvtps_epi32(_mm_mul_ps(_mm_add_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+12)), one), one), mulby127));  /* load 4 floats, clamp, convert to sint32 */
...
SDL_Convert_F32_to_S16_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    if ((((size_t) StreamMedia) & 15) == 0) {
        ...
        __m128i *mmdst = (__m128i *) dst;
        while (i >= 8) {   /* 8 * float32 */
            const __m128i ints1 = _mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia)), one), mulby32767));  /* load 4 floats, clamp, convert to sint32 */
            const __m128i ints2 = _mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia+4)), one), mulby32767));  /* load 4 floats, clamp, convert to sint32 */
            _mm_store_si128(mmdst, _mm_packs_epi32(ints1, ints2));  /* pack to sint16, store out. */
            i -= 8; StreamMedia += 8; mmdst++;
        }
        dst = (Sint16 *) mmdst;
    }
...
SDL_Convert_F32_to_S32_SSE2(SDL_AudioCVT *cvt, SDL_AudioFormat format)
{
    ...
    SDL_assert(!i || ((((size_t) dst) & 15) == 0));
    SDL_assert(!i || ((((size_t) StreamMedia) & 15) == 0));

    {
        ...
        __m128i *mmdst = (__m128i *) dst;
        while (i >= 4) {   /* 4 * float32 */
            _mm_store_si128(mmdst, _mm_slli_epi32(_mm_cvtps_epi32(_mm_mul_ps(_mm_min_ps(_mm_max_ps(negone, _mm_load_ps(StreamMedia)), one), mulby8388607)), 8));  /* load 4 floats, clamp, convert to sint32 */
            i -= 4; StreamMedia += 4; mmdst++;
        }
        dst = (Sint32 *) mmdst;
    }
