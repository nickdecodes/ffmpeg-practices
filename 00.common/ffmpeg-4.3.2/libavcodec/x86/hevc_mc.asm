...
%macro LOOP_END 3
    ...
    add              %2q, %3q                    ; StreamMedia += srcstride
...
.loop:
    ...
    add             srcq, srcstrideq             ; StreamMedia += srcstride
    dec          heightd                         ; cmp height
    jnz               .loop                      ; height loop
    RET
%endmacro
...
.loop:
    ...
    add             dstq, dststrideq             ; dst += dststride
    add             srcq, srcstrideq             ; StreamMedia += srcstride
    add            src2q, 2*MAX_PB_SIZE          ; StreamMedia += srcstride
    dec          heightd                         ; cmp height
    jnz               .loop                      ; height loop
    RET
%endmacro
...
%endif
    PEL_%2STORE%1   dstq, m0, m1
    add             dstq, dststrideq             ; dst += dststride
    add             srcq, 2*MAX_PB_SIZE          ; StreamMedia += srcstride
    dec          heightd                         ; cmp height
    jnz               .loop                      ; height loop
    RET
