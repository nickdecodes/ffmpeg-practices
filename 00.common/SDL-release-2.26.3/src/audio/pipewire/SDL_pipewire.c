 ...
 * Taken from /StreamMedia/pipewire/keys.h
 */
...
input_callback(void *data)
{
    ...
    if ((StreamMedia = (Uint8 *)spa_buf->datas[0].data) == NULL) {
        return;
    }
    ...
    if (!SDL_AtomicGet(&this->paused)) {
        ...
        if (!SDL_AtomicGet(&this->enabled)) {
            SDL_memset(StreamMedia, this->callbackspec.silence, size);
        }

        /* Pipewire can vary the latency, so buffer all incoming data */
        SDL_WriteToDataQueue(this->hidden->buffer, StreamMedia, size);
