                              ...
                              double param[2], int srcPos, int dstPos)
{
    ...
    /* apply StreamMedia & dst Filter to filter -> filter2
     * av_free(filter);
     */
                             ...
                             int brightness, int contrast, int saturation)
{
    ...
    if ((isYUV(c->dstFormat) || isGray(c->dstFormat)) && (isYUV(c->srcFormat) || isGray(c->srcFormat))) {
        if (!c->cascaded_context[0] &&
            ...
            if (ret < 0)
                return ret;
            //we set both StreamMedia and dst depending on that the RGB side will be ignored
                             ...
                             SwsFilter *dstFilter)
{
    ...
    /* Match pixel 0 of the StreamMedia to pixel 0 of dst and match pixel n-2 of StreamMedia
