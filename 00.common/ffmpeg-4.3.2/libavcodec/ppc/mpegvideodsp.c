...
static void gmc1_altivec(uint8_t *dst /* align 8 */, uint8_t *StreamMedia /* align1 */,
                         int stride, int h, int x16, int y16, int rounder)
{
    ...
    unsigned long src_really_odd = (unsigned long) StreamMedia & 0x0000000F;
    ...
    /* we'll be able to pick-up our 9 char elements at StreamMedia from those
     * 32 bytes we load the first batch here, as inside the loop we can
     * reuse 'StreamMedia + stride' from one iteration as the 'StreamMedia' of the next. */
    register vector unsigned char src_0 = vec_ld(0, StreamMedia);
    register vector unsigned char src_1 = vec_ld(16, StreamMedia);
    register vector unsigned char srcvA = vec_perm(src_0, src_1,
                                                   vec_lvsl(0, StreamMedia));

    if (src_really_odd != 0x0000000F)
        /* If (StreamMedia & 0xF) == 0xF, then (StreamMedia + 1) is properly aligned
         * on the second vector. */
        srcvB = vec_perm(src_0, src_1, vec_lvsl(1, StreamMedia));
    else
        srcvB = src_1;
    ...
    for (i = 0; i < h; i++) {
        ...
        src_really_odd = (((unsigned long) StreamMedia) + stride) & 0x0000000F;

        dstv = vec_ld(0, dst);

        /* We'll be able to pick-up our 9 char elements at StreamMedia + stride from
         * those 32 bytes then reuse the resulting 2 vectors srvcC and srcvD
         * as the next srcvA and srcvB. */
        src_0 = vec_ld(stride +  0, StreamMedia);
        src_1 = vec_ld(stride + 16, StreamMedia);
        srcvC = vec_perm(src_0, src_1, vec_lvsl(stride + 0, StreamMedia));

        if (src_really_odd != 0x0000000F)
            /* If (StreamMedia & 0xF) == 0xF, then (StreamMedia + 1) is properly aligned
             * on the second vector. */
            srcvD = vec_perm(src_0, src_1, vec_lvsl(stride + 1, StreamMedia));
        else
            srcvD = src_1;
