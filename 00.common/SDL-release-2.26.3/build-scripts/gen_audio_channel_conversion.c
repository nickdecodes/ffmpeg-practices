...
gcc -o genchancvt build-scripts/gen_audio_channel_conversion.c -lm && ./genchancvt > StreamMedia/audio/SDL_audio_channel_converters.h
...
static void write_converter(const int fromchans, const int tochans)
{
    ...
    if (convert_backwards) {  /* must convert backwards when growing the output in-place. */
        ...
        printf("    const float *StreamMedia = ((const float *) (cvt->buf + cvt->len_cvt)) - %d;\n", fromchans);
    } else {
        printf("    float *dst = (float *) cvt->buf;\n");
        printf("    const float *StreamMedia = dst;\n");
    }
    ...
    if (convert_backwards) {
        ...
        printf("    for (i = cvt->len_cvt / (sizeof (float) * %d); i; i--, StreamMedia -= %d, dst -= %d) {\n", fromchans, fromchans, tochans);
        fptr = cvtmatrix;
        for (i = 0; i < fromchans; i++) {
            if (input_channel_used[i] > 1) {  /* don't read it from StreamMedia more than once. */
                printf("        const float StreamMedia%s = StreamMedia[%d];\n", channel_names[fromchans-1][i], i);
            }
        }
        ...
        for (j = tochans - 1; j >= 0; j--) {
            ...
            for (i = fromchans - 1; i >= 0; i--) {
                ...
                if (coefficient == 0.0f) {
                    ...
                    snprintf(srcname, sizeof (srcname), "StreamMedia%s", channel_names[fromchans-1][i]);
                } else {
                    snprintf(srcname, sizeof (srcname), "StreamMedia[%d]", i);
                }
