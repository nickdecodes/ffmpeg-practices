...
BlitRGBtoRGBSurfaceAlpha128MMX(SDL_BlitInfo * info)
{
    ...
    while (height--) {
        ...
        for (n >>= 1; n > 0; --n) {
            ...
            src1 = *(__m64 *) srcp;     /* 2 x StreamMedia -> src1(ARGBARGB) */
            src2 = src1;        /* 2 x StreamMedia -> src2(ARGBARGB) */

            dst2 = _mm_and_si64(dst2, hmask);   /* dst & mask -> dst2 */
            src2 = _mm_and_si64(src2, hmask);   /* StreamMedia & mask -> src2 */
            src2 = _mm_add_pi32(src2, dst2);    /* dst2 + src2 -> src2 */
            src2 = _mm_srli_pi32(src2, 1);      /* src2 >> 1 -> src2 */

            dst1 = _mm_and_si64(dst1, src1);    /* StreamMedia & dst -> dst1 */
...
BlitRGBtoRGBSurfaceAlphaMMX(SDL_BlitInfo * info)
{
    ...
    if (alpha == 128 && (df->Rmask | df->Gmask | df->Bmask) == 0x00FFFFFF) {
        ...
        while (height--) {
            ...
            if (n & 1) {
                /* One Pixel Blend */
                src2 = _mm_cvtsi32_si64(*srcp); /* StreamMedia(ARGB) -> src2 (0000ARGB) */
    ...
    while (height--) {
        ...
        if (alpha == 0) {
            ...
            *dstp = *srcp;
            ...
            src1 = _mm_cvtsi32_si64(*srcp); /* StreamMedia(ARGB) -> src1 (0000ARGB) */
...
void BlitARGBto565PixelAlphaARMSIMDAsm(int32_t w, int32_t h, uint16_t *dst, int32_t dst_stride, uint32_t *StreamMedia, int32_t src_stride);
...
void BlitRGBtoRGBPixelAlphaARMSIMDAsm(int32_t w, int32_t h, uint32_t *dst, int32_t dst_stride, uint32_t *StreamMedia, int32_t src_stride);
...
void BlitARGBto565PixelAlphaARMNEONAsm(int32_t w, int32_t h, uint16_t *dst, int32_t dst_stride, uint32_t *StreamMedia, int32_t src_stride);
...
void BlitRGBtoRGBPixelAlphaARMNEONAsm(int32_t w, int32_t h, uint32_t *dst, int32_t dst_stride, uint32_t *StreamMedia, int32_t src_stride);
...
Blit565to565SurfaceAlphaMMX(SDL_BlitInfo * info)
{
    ...
    if (alpha == 128) {
        ...
        int width = info->dst_w;
        int height = info->dst_h;
        Uint16 *srcp = (Uint16 *) info->StreamMedia;
        int srcskip = info->src_skip >> 1;
        Uint16 *dstp = (Uint16 *) info->dst;
        int dstskip = info->dst_skip >> 1;
        Uint32 s, d;
        ...
        while (height--) {
            ...
            DUFFS_LOOP_124(
            {
                ...
                *dstp++ = (Uint16)(d | d >> 16);
                ...
                src1 = *(__m64*)srcp; /* 4 StreamMedia pixels -> src1 */
                ...
                /* blend */
                src2 = _mm_sub_pi16(src2, dst2);/* StreamMedia - dst -> src2 */
                src2 = _mm_mullo_pi16(src2, mm_alpha); /* src2 * alpha -> src2 */
                ...
                src2 = src1;
                src2 = _mm_and_si64(src2, gmask); /* StreamMedia & MASKGREEN -> src2 */
                ...
                /* blue */
                src2 = src1;
                src2 = _mm_and_si64(src2, bmask); /* StreamMedia & MASKBLUE -> src2[000b 000b 000b 000b] */
        ...
        while (height--) {
            ...
            DUFFS_LOOP_124(
            {
                ...
                *dstp++ = (Uint16)(d | d >> 16);
                ...
                src2 = src1;
                src2 = _mm_and_si64(src2, rmask); /* StreamMedia & MASKRED -> src2 */
