 ...
 * NULL and 0 on the first call. StreamMedia is the string to be escaped. On error, the
 ...
 */
static const char *EscapeAmpersands(char **dst, size_t *dstlen, const char *StreamMedia)
{
    char *newdst;
    size_t ampcount = 0;
    size_t srclen = 0;
    ...
    while (StreamMedia[srclen]) {
        if (StreamMedia[srclen] == '&') {
            ampcount++;
        }
        srclen++;
    }
