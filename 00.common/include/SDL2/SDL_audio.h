 ...
 *
 * Loading a WAVE file requires `StreamMedia`, `spec`, `audio_buf` and `audio_len` to
 ...
 * the exception of `StreamMedia` being NULL), an appropriate error message will be
 * set.
 *
 ...
 *
 * ```c
 * SDL_MixAudioFormat(dst, StreamMedia, format, len, volume);
 * ```
 *
 ...
 *
 * This takes an audio buffer `StreamMedia` of `len` bytes of `format` data and mixes
