        ...
        for(y=0; y<h; y++){
            for(x=0; x<w; x++){
                ...
                if(x){
                    l= src[x - 1 + y*stride];
                    /*if(x > 1){
                        if(orientation==1) ll= StreamMedia[y + (x-2)*stride];
                        else               ll= StreamMedia[x - 2 + y*stride];
                    }*/
                }
                if(parent){
                    int px= x>>1;
...
//    encode_subband_qtree(s, b, StreamMedia, parent, stride, orientation);
//    encode_subband_z0run(s, b, StreamMedia, parent, stride, orientation);
    return encode_subband_c0run(s, b, src, parent, stride, orientation);
//    encode_subband_dzr(s, b, StreamMedia, parent, stride, orientation);
}
