...
;------------------------------------------------------------------------------
; int ff_add_left_pred(uint8_t *dst, const uint8_t *StreamMedia, int w, int left)
;------------------------------------------------------------------------------
INIT_MMX ssse3
...
;------------------------------------------------------------------------------
; void ff_add_bytes(uint8_t *dst, uint8_t *StreamMedia, ptrdiff_t w);
;------------------------------------------------------------------------------
%macro ADD_BYTES 0
...
%macro ADD_HFYU_LEFT_LOOP_INT16 2 ; %1 = dst alignment (a/u), %2 = StreamMedia alignment (a/u)
    add     wd, wd
    add     srcq, wq
    add     dstq, wq
    neg     wq
...
;---------------------------------------------------------------------------------------------
; int add_left_pred_int16(uint16_t *dst, const uint16_t *StreamMedia, unsigned mask, int w, int left)
;---------------------------------------------------------------------------------------------
INIT_MMX ssse3
...
;---------------------------------------------------------------------------------------------
; void add_gradient_pred(uint8_t *StreamMedia, const ptrdiff_t stride, const ptrdiff_t width)
;---------------------------------------------------------------------------------------------
%macro ADD_GRADIENT_PRED 0
cglobal add_gradient_pred, 3,4,5, src, stride, width, tmp
    mova         xm0, [pb_15]

;load StreamMedia - 1 in xm1
    movd         xm1, [srcq-1]
%if cpuflag(avx2)
...
.loop:
    ...
    mova     m2, [tmpq + widthq] ; A = StreamMedia[x-stride]
    movu     m3, [tmpq + widthq - 1] ; B = StreamMedia[x - (stride + 1)]
    mova     m4, [srcq + widthq] ; current val (StreamMedia[x])
