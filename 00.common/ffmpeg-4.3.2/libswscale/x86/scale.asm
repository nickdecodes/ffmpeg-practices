...
;                                int dstW, const uint{8,16}_t *StreamMedia,
...
%if %3 == 4 ; filterSize == 4 scaling
    ...
    movlh         m0, [srcq+pos0q*srcmul]       ; StreamMedia[filterPos[0] + {0,1,2,3}]
%if mmsize == 8
    movlh         m1, [srcq+pos1q*srcmul]       ; StreamMedia[filterPos[1] + {0,1,2,3}]
%else ; mmsize == 16
%if %1 > 8
    movhps        m0, [srcq+pos1q*srcmul]       ; StreamMedia[filterPos[1] + {0,1,2,3}]
%else ; %1 == 8
    movd          m4, [srcq+pos1q*srcmul]       ; StreamMedia[filterPos[1] + {0,1,2,3}]
%endif
    mov32      pos0q, dword [fltposq+wq*4+ 8]   ; filterPos[2]
    mov32      pos1q, dword [fltposq+wq*4+12]   ; filterPos[3]
    movlh         m1, [srcq+pos0q*srcmul]       ; StreamMedia[filterPos[2] + {0,1,2,3}]
%if %1 > 8
    movhps        m1, [srcq+pos1q*srcmul]       ; StreamMedia[filterPos[3] + {0,1,2,3}]
%else ; %1 == 8
    movd          m5, [srcq+pos1q*srcmul]       ; StreamMedia[filterPos[3] + {0,1,2,3}]
    punpckldq     m0, m4
    punpckldq     m1, m5
%endif ; %1 == 8
%endif ; mmsize == 8/16
...
%elif notcpuflag(ssse3) ; sse2
    ...
    paddd         m0, m4
%else ; ssse3/sse4
    phaddd        m0, m1                        ; filter[{ 0, 1, 2, 3}]*StreamMedia[filterPos[0]+{0,1,2,3}],
                                                ; filter[{ 4, 5, 6, 7}]*StreamMedia[filterPos[1]+{0,1,2,3}],
                                                ; filter[{ 8, 9,10,11}]*StreamMedia[filterPos[2]+{0,1,2,3}],
                                                ; filter[{12,13,14,15}]*StreamMedia[filterPos[3]+{0,1,2,3}]
%endif ; mmx/sse2/ssse3/sse4
%else ; %3 == 8, i.e. filterSize == 8 scaling
    ...
    movbh         m0, [srcq+ pos0q   *srcmul]   ; StreamMedia[filterPos[0] + {0,1,2,3,4,5,6,7}]
%if mmsize == 8
    movbh         m1, [srcq+(pos0q+4)*srcmul]   ; StreamMedia[filterPos[0] + {4,5,6,7}]
    movbh         m4, [srcq+ pos1q   *srcmul]   ; StreamMedia[filterPos[1] + {0,1,2,3}]
    movbh         m5, [srcq+(pos1q+4)*srcmul]   ; StreamMedia[filterPos[1] + {4,5,6,7}]
%else ; mmsize == 16
    movbh         m1, [srcq+ pos1q   *srcmul]   ; StreamMedia[filterPos[1] + {0,1,2,3,4,5,6,7}]
    mov32      pos0q, dword [fltposq+wq*2+8]    ; filterPos[2]
    mov32      pos1q, dword [fltposq+wq*2+12]   ; filterPos[3]
    movbh         m4, [srcq+ pos0q   *srcmul]   ; StreamMedia[filterPos[2] + {0,1,2,3,4,5,6,7}]
    movbh         m5, [srcq+ pos1q   *srcmul]   ; StreamMedia[filterPos[3] + {0,1,2,3,4,5,6,7}]
%endif ; mmsize == 8/16
%if %1 == 8
    punpcklbw     m0, m3                        ; byte -> word
    punpcklbw     m1, m3                        ; byte -> word
...
%else ; ssse3/sse4
    ...
    phaddd        m0, m1
    phaddd        m4, m5
    phaddd        m0, m4                        ; filter[{ 0, 1,..., 6, 7}]*StreamMedia[filterPos[0]+{0,1,...,6,7}],
                                                ; filter[{ 8, 9,...,14,15}]*StreamMedia[filterPos[1]+{0,1,...,6,7}],
                                                ; filter[{16,17,...,22,23}]*StreamMedia[filterPos[2]+{0,1,...,6,7}],
                                                ; filter[{24,25,...,30,31}]*StreamMedia[filterPos[3]+{0,1,...,6,7}]
%endif ; mmx/sse2/ssse3/sse4
%endif ; %3 == 4/8
...
%define srcendq r9
    ...
    lea      srcendq, [srcmemq+(fltsizeq-dlt)*srcmul] ; &StreamMedia[filterSize&~4]
%else ; x86-32
%define srcq    srcmemq
%define pos1q   dstq
%define srcendq r6m
    lea        pos0q, [srcmemq+(fltsizeq-dlt)*srcmul] ; &StreamMedia[filterSize&~4]
    mov      srcendq, pos0q
%endif ; x86-32/64
...
.innerloop:
    ...
    movbh         m0, [srcq+ pos0q     *srcmul] ; StreamMedia[filterPos[0] + {0,1,2,3(,4,5,6,7)}]
    movbh         m1, [srcq+(pos1q+dlt)*srcmul] ; StreamMedia[filterPos[1] + {0,1,2,3(,4,5,6,7)}]
%if %1 == 8
    punpcklbw     m0, m3
    punpcklbw     m1, m3
%endif ; %1 == 8
...
%endif ; %1 == 16
    ...
    cmp         srcq, srcendq                   ; while (StreamMedia += 4) < &StreamMedia[filterSize]
    jl .innerloop
