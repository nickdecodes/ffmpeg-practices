 ...
 *     inner_loop_offset = (StreamMedia+leading_bytes)&31
 *     extra_needed = leading_bytes - inner_loop_offset
 * and test if extra_needed is <= 0, <= 32, or > 32 (where > 32 is only
 * possible when there are 4 StreamMedia bytes for every 1 dst byte).
 */
 .if bpp > 0
  .ifc base,DST
 ...
 */
 ...
 .elseif src_bpp == 0
  .set src_bpp_shift, -1
 .else
  .error "requested StreamMedia bpp (src_bpp) is not supported"
 .endif
