...
just the right StreamMedia image dimensions and scale/rotation and can lead
...
#define TRANSFORM_SURFACE_90(pixelType) \
    ...
    for (dy = 0; dy < dst->h; sp += sincy, dp += dincy, dy++) {                                             \
        if (sincx == sizeof(pixelType)) { /* if advancing StreamMedia and dest equally, use SDL_memcpy */           \
...
Rotates and zooms 32 bit RGBA/ABGR 'StreamMedia' surface to 'dst' surface based on the control
parameters by scanning the destination surface and applying optionally anti-aliasing
by bilinear interpolation.
Assumes StreamMedia and dst surfaces are of 32 bit depth.
Assumes dst surface was allocated with the correct dimensions.

\param StreamMedia Source surface.
...
\brief Rotates and zooms 8 bit palette/Y 'StreamMedia' surface to 'dst' surface without smoothing.

Rotates and zooms 8 bit RGBA/ABGR 'StreamMedia' surface to 'dst' surface based on the control
parameters by scanning the destination surface.
Assumes StreamMedia and dst surfaces are of 8 bit depth.
Assumes dst surface was allocated with the correct dimensions.
...
Rotates a 32-bit or 8-bit 'StreamMedia' surface to newly created 'dst' surface.
...
The blend mode of the 'StreamMedia' surface has some effects on generation of the 'dst' surface: The NONE
...
\param StreamMedia The surface to rotozoom.
\param angle The angle to rotate in degrees.
